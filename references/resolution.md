...
|---|---|
| `commands.install` / `.test` / `.lint` / `.typecheck` / `.format` / `.run` | Each resolved **independently**: Tier A project-authored task runners (`Makefile`, `justfile`, `Taskfile.yml`, `mise.toml` — emit `make <target>` only for targets that exist) → Tier B language manifests (`package.json`+lockfile, `pyproject.toml`, `Cargo.toml`, `go.mod`, `mix.exs`, `Gemfile`, Gradle, `pom.xml`, `composer.json`, `*.csproj`) → Tier C `.github/workflows/*.yml` `run:` steps, treated as a suggestion and confirmed with the user, never run blind. If nothing in any tier resolves and verifies (head binary must resolve; for `make`/`just`/`task` the target must exist): **`null` — the corresponding gate is skipped with a one-line note, not an error** |
