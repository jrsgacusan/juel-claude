...
| `docsRoot` | `<CWD_ROOT>/docs/.superpowers/` if it already exists and is non-empty (keeps an existing repo's prior specs/plans/context on the dotted path); otherwise `<CWD_ROOT>/docs/superpowers/` for a brand-new repo. Resolved **once** and reused — a skill must never pick between the two variants ad hoc. **Never overwrite an existing file under `${docsRoot}`** — a spec, plan, findings report, or context file. On a name collision, append `-v2` before the extension; if `-v2` exists too, use `-v3`, and so on. This applies to every file type written under `${docsRoot}`, not only the one a given skill produces |
